...
class S3ToSnowflakePipeOperator(BaseOperator):
    ...
    def list_files_from_s3(self):
        ...
        listing = hook.list_keys(bucket_name=self.s3_bucket, prefix=self.file_prefix)
    ...
    def execute(self, context):
        ...
        if files:
            self.log.debug('sending {0} files to {1}'.format(len(files),self.pipe))
            hook.pipe_insert_files(self.pipe,files,schema='analysis')
        else:
            self.log.info("didn't find any files like {0}/{1}".format(self.s3_bucket, self.file_prefix))
